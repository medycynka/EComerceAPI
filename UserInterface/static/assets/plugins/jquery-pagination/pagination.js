...
(function ($) {
    ...
    $.fn.rpmPagination = function (options) {
        ...
        if (rpmPageTotal > settings.limit) {
            if (settings.forceTotalPages === false) {
        ...
        preparePageMenus(settings.currentPage, pages, $this, settings.forceTotalPages);
            ...
            if ($(this).parent().hasClass('disabled') || $(this).parent($this).length === 0) {
                ...
                if (settings.refresh === true) {
                    ...
                    if($(this).parent().hasClass('first')){
                        ...
                        settings.currentPage = rpmPageTotal;
                        [settings.currentPage, rpmPageNext] = preparePageItems(settings.currentPage, rpmPageNext, settings.limit, rpmCustomDomElem, 'page-item')
     ...
     * @returns {boolean} true
     */
    var preparePageMenus = function (current_page, pages, element, forcePages) {

        $(element).html('');
        let pageArray = [], fp = 1, lp = (forcePages === false ? (pages - 1) : pages);
        let menuHtml = '';

        if (forcePages){
            if (lp < 5){
                pageArray = Array.from({length: lp}, (x, i) => i + 1)
            } else if(lp === 5) {
                pageArray = [1, 2, '...', lp - 1, lp]
            } else {
        ...
        menuHtml = '<li class="' + rpmListClass + ' first ' + (current_page === fp ? 'disabled' : '') + '"><a class="' + rpmAnchorClass + '" href="#"><<</a></li>';
        ...
        for (let i = 0; i < pageArray.length; i++) {
            if (lp < i) {
                ...
                if (pageArray[i] === '...') {
                    ...
                    menuHtml += '<li class="' + rpmListClass + ' page-' + pageArray[i] + ' ' + (pageArray[i] === current_page ? 'disabled' : '') + (window.onlyTwoCards === true && pageArray[i] === 3 ? 'hide': '') + '"><a class="' + rpmAnchorClass + '" data-page_no="' + pageArray[i] + '" href="#">' + pageArray[i] + '</a></li>';
                }
            }
        }
        menuHtml += '<li class="' + rpmListClass + ' next ' + (current_page === lp ? 'disabled' : '') + '"><a class="' + rpmAnchorClass + '" href="#">></a></li>';
        menuHtml += '<li class="' + rpmListClass + ' last ' + (current_page === lp ? 'disabled' : '') + '"><a class="' + rpmAnchorClass + '" href="#">>></a></li>';
