	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			...
			var RemoveFile = function(e) {
				...
				if(settings.fileRemoveOption !== undefined){
					settings.fileRemoveOption(data.files[0].name)
				}
			...
			{
				var found = false;
				if(settings.accept.length > 0 && settings.accept[0] !== '*') {
				...
				if (!found)  data.ff_info.errors.push(Translate('Invalid file extension.') + Translate('Accepted extensions: ') + settings.accept.join(', '));
			}
			...
			if (settings.edit && !data.ff_info.errors.length)
			{
				inforow.find('.ff_fileupload_filename').append($('<input style="border: 1px solid #eaedf1; border-radius: 5px; padding: 7px; max-width: 50%;">').attr('type', 'text').val(filename).keydown(function(e) {
					// Start uploading if someone presses enter.
					if (e.keyCode == 13) {
						if(settings.allowUpload === undefined && settings.allowUpload === true) {
							StartUpload(e);
						} else {
							e.preventDefault()
						}
		...
		return this.each(function() {
			...
			if (settings.initialInput !== undefined && settings.initialInput !== null){
				console.log(settings.initialInput)
				fileinput.fileupload('add', {files: settings.initialInput[0].files})
			}
