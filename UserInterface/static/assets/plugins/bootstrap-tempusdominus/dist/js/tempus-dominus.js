    ...
    class DateDisplay {
        ...
        _update(widget, paint) {
            ...
            container
                ...
                const cellData = `${innerDate.year}-${innerDate.monthFormatted}-${innerDate.dateFormatted}`
                containerClone.setAttribute('data-value', cellData);
                containerClone.setAttribute('data-day', `${innerDate.date}`);

                if(this.optionsStore.options && this.optionsStore.options.meta && this.optionsStore.options.meta.additionalDateMap) {
                    if (this.optionsStore.options.meta.additionalDateMap[cellData] !== undefined && this.optionsStore.options.meta.additionalDateMap[cellData] > 0) {
                        const eventsInDate = this.optionsStore.options.meta.additionalDateMap[cellData]

                        containerClone.innerHTML = innerDate.format({day: 'numeric'}) + `<span class="calendar-events-count">${eventsInDate}</span>`;
                        containerClone.setAttribute('data-bs-placement', 'top')
                        containerClone.setAttribute('data-bs-toggle', 'tooltip')
                        containerClone.setAttribute('title', `Ilość wydarzeń w tym dniu: ${eventsInDate}`)
                    } else {
                        containerClone.innerHTML = innerDate.format({day: 'numeric'});
                        containerClone.setAttribute('data-placement', 'top')
                        containerClone.setAttribute('data-toggle', 'tooltip')
                        containerClone.setAttribute('title', `Brak wydarzeń w tym dniu`)
                    }
                } else {
                    containerClone.innerHTML = innerDate.format({day: 'numeric'});
                }

                containerClone.addEventListener('mouseenter', function (e) {
                    const tooltips = $(".tooltip")
                    if(tooltips.length > 0) {
                        tooltips.tooltip('hide')
                    }

                    let tooltip =  new bootstrap.Tooltip(e.target, {
                        template: '<div class="tooltip tooltip-primary" role="tooltip" style=""><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>',
                        placement: 'auto',
